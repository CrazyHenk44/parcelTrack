...
class YunExpressShipper implements ShipperInterface
{
    ...
    public function fetch(string $trackingCode, string $postalCode, string $country): ?TrackingResult
    {
        ...
        $res = $this->getTrackingResult($trackingCode, $postalCode, $country, $response);
        return $res;
    }

    public function getTrackingResult(string $trackingCode, string $postalCode, string $country, ?string $apiResponse): ?TrackingResult
    {
        if ($apiResponse === null) {
            return null;
        }
